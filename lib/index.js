 ...
 * 通过用户配置的 Git 仓库在【多台电脑】之间双向同步，推荐使用私有仓库。
 * 另提供归档会话管理与历史冲突副本处理界面。
 *
 * 配置:读 DSH home 下的 dsh-sync.json(remote 为 GitHub 私有仓库地址);
 *       mode=manual(默认,纯手动)/ auto(自动同步);设置页切换自动模式即时生效。
 */
import fs from 'node:fs';
import os from 'node:os';
import path from 'node:path';
import { SyncEngine, DEFAULT_CONFIG, decodeGroup, normPath, humanSize, ZSTD_MAGIC, decodeZstdFrames, SESSION_LOG_NAMES } from './sync.js';
...
function summarize(r) {
  ...
  if (r.conflicts && r.conflicts.length) parts.push(r.conflicts.length + ' 个冲突已自动合并(零人工裁决)');
  if (r.backupBranch) parts.push('远端备份: ' + r.backupBranch + (r.backupPushed ? '（已上传）' : '（仅本机）'));
...
function lastDetail(r) {
  ...
  return {
    ...
    backupPushed: r.backupPushed === true,
    workspaces: w ? {
      total: w.total || 0,
      synced: w.synced || 0,
      pushed: w.pushed || 0,
      ...
      backups: Array.isArray(w.backups) ? w.backups.map((b) => ({ id: b.id, branch: b.branch, pushed: b.pushed === true })) : [],
...
function apply(ctx, config = {}) {
  ...
  const scheduleFlush = () => {
    ...
    // 兜底刷新定时器不阻止进程退出(退出冲刷由客户端 /api/session/flush 与 ctx.effect 负责)
    flushTimer.unref?.();
  };
  ...
  const scheduler = createSyncScheduler({
    ...
    sync: (reason) => engine.syncOnce(reason),
    afterSync: () => runPatches('after-sync'),
    log,
  });
  ...
  try {
    disposers.push(ctx.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          json(res, 200, { ok: !r.error && !(r.workspaces?.errors?.length), summary: summarize(r), detail: r });
        } catch (e) {
    ...
    disposers.push(ctx.webServer.register({
      ...
      handler: (req, res) => {
        ...
        json(res, 200, {
          ...
          lastOk: engine.lastOutcome ? !engine.lastOutcome.error && !engine.lastOutcome.workspaces?.errors?.length : null,
    ...
    disposers.push(ctx.webServer.register({
      ...
      handler: async (req, res) => {
        try {
          ...
          scheduler.refresh();
          const effective = engine.loadConfig();
          json(res, 200, { ok: true, saved: patch, auto: scheduler.active, config: { mode: effective.mode, workspaceSync: effective.workspaceSync !== false, enabled: effective.enabled !== false } });
        } catch (e) {
          json(res, 200, { ok: false, error: String((e && e.message) || e) });
        }
      },
    ...
    // —— 历史冲突拷贝审计(只读):新版分层合并引擎已不再生成冲突拷贝,这里仅列出遗留项供清理参考 ——
    disposers.push(ctx.webServer.register({
      kind: 'exact',
  ...
  // —— 历史冲突拷贝审计工具(只读):新版分层合并引擎不再生成冲突拷贝,仅用于查看/清理遗留项 ——
  const toolOut = { schema: { type: 'string' }, render: (_a, v) => [{ type: 'text', text: String(v) }] };
  try {
    disposers.push(ctx.tools.register({
      name: 'sync_conflicts',
      description: '只读列出历史遗留的冲突拷贝(.dsh-conflict-<时间戳>,主仓库或某个工作区)。新版同步已自动合并冲突、不再生成新拷贝,此工具仅用于审计/清理旧版本留下的副本,不用于人工裁决。',
      parameters: {},
      output: toolOut,
      async execute() {
        const list = engine.outstandingConflicts();
        if (!list.length) return '(没有遗留的冲突拷贝 —— 新版同步已自动合并,无需人工裁决)';
        return list.map((c, i) =>
    ...
    log('warn', '冲突拷贝审计工具注册失败: ' + String((err && err.message) || err));
  }

  // Register once; the scheduler gates activity and replaces timers when preferences change.
  try { disposers.push(ctx.on('session/event', () => scheduler.onActivity())); }
  catch { log('info', 'ctx.on(session/event) 不可用,仅按周期同步'); }
  scheduler.refresh({ startup: true });
  ctx.effect(() => () => {
    const current = engine.loadConfig();
    scheduler.stop();
    clearPendingFlush();
    for (const d of disposers) { try { d(); } catch { /* ignore */ } }
    if (current.mode === 'auto' && current.enabled !== false && current.autoPushOnExit !== false) {
      try { engine.flushSync(); log('info', '退出冲刷完成'); }
      catch (e) { log('warn', `退出冲刷失败: ${e?.message || e}`); }
    }
  });
