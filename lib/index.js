...
import { SyncEngine, DEFAULT_CONFIG, decodeGroup, normPath } from './sync.js';
...
function apply(ctx, config = {}) {
  ...
  // 删除/取消归档等登记表改动后,立即提交+推送:否则下次拉取会把旧登记又带回来(删除"复活")
  const scheduleFlush = () => {
    try {
      setImmediate(() => {
        try { engine.flushSync(); }
        catch (e) { log('warn', '操作后立即同步失败(下次手动同步会补上): ' + String((e && e.message) || e)); }
      });
    } catch { /* ignore */ }
  };
    ...
    disposers.push(ctx.webServer.register({
      ...
      handler: (req, res) => {
        try {
          ...
          // 会话日志冲突:补上原标题(从投影缓存读),让用户能判断保留哪一侧
    ...
    disposers.push(ctx.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          json(res, 200, { ok: true, sessionId: idHint, local, remote });
        } catch (e) {
          json(res, 200, { ok: false, error: String((e && e.message) || e) });
        }
      },
