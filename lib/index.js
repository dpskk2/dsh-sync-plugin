/**
 * dsh-sync —— DeepSeek Harness 一键同步 + 会话管理插件(宿主端主入口)
 *
 * 安装(一行命令):
...
function apply(ctx, config = {}) {
  ...
  try {
    ...
    disposers.push(ctx.tools.register({
      name: 'session_delete',
      description: '删除(移入本地回收站)指定 DSH 会话。数据可在 ~/.dsh/.trash 找回;同步后其他电脑上的同一会话也会被删除。执行前先用 session_list 找到确切的 id,并和用户确认。',
      parameters: {
      ...
      async execute(args) {
        ...
        return `已删除会话 ${r.id}(所在组: ${r.groups.join(', ')})。回收站: ${r.trash}。页面侧栏将在刷新/重启后不再显示;下次点「同步」会把删除传播到其他电脑。`;
      },
    }));
  } catch (err) {
  ...
  log('info', `dsh-sync 已启动(mode=${cfg.mode || 'manual'}, home=${home}, 分支=${cfg.branch}, remote=${cfg.remote || '(未配置,仅本地快照)'}, UI=侧栏「⟳ 同步」按钮 + 设置「同步 · 会话管理」)`);
}
