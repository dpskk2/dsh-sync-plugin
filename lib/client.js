    ...
    function SyncPreferences({ busy, onSaved }) {
      ...
      const save = async (event) => {
        ...
        try {
          ...
          setMessage(overridden ? '已保存，但宿主加载参数覆盖了部分偏好；下方显示实际配置。' : '已保存，同步方式立即生效，无需重启。工作区选项在下次同步时使用；正在进行的同步会继续完成。');
          if (onSaved) onSaved();
      ...
      return e('form', { className: 'dss-sec', onSubmit: save },
        ...
        config ? e('fieldset', { disabled: saving || busy, style: { border: 0, margin: 0, padding: 0, minWidth: 0 } },
          ...
          e('div', { className: 'dss-desc' }, '默认手动。自动模式默认每 5 分钟同步，并响应会话活动；保存后立即生效，无需重启。'),
    ...
    function Section(props) {
      ...
      const statusDesc = (() => {
        ...
        return status.remote + '\n'
          + '分支 ' + status.branch + ' · ' + (status.auto ? '自动同步运行中' : '自动同步未运行')
