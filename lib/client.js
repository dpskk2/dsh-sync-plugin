...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function lastDetailText(lastDetail) {
      ...
      if (lastDetail.backupBranch) lines.push('备份分支: ' + lastDetail.backupBranch + '(可能仅在本机，请检查备份推送日志)');
    ...
    /* 常用偏好复用已有配置 API；自动调度在宿主启动时注册，保存后明确提示重启。 */
    function SyncPreferences({ busy, onSaved }) {
      ...
      const save = async (event) => {
        ...
        setMessage('');
        try {
          const d = await API.saveConfig({ mode: config.mode, workspaceSync: config.workspaceSync });
          if (!d || !d.ok) throw new Error((d && d.error) || '保存失败，请重试');
          setMessage('已保存。工作区选项在下次同步时使用；更改手动 / 自动模式后，请重启 DSH。');
          if (onSaved) onSaved();
        } catch (err) { setError(err.message || String(err)); }
        finally { setSaving(false); }
      };
      return e('form', { className: 'dss-sec', onSubmit: save },
        e('div', { className: 'dss-h' }, '同步偏好'),
        ...
        config ? e('fieldset', { disabled: saving || busy, style: { border: 0, margin: 0, padding: 0, minWidth: 0 } },
          e('label', { className: 'dss-rowline', style: { marginTop: '12px', flexWrap: 'wrap' } },
            ...
            e('select', {
              ...
              onChange: (event) => { setConfig({ ...config, mode: event.target.value }); setMessage(''); },
            }, e('option', { value: 'manual' }, '手动同步'), e('option', { value: 'auto' }, '自动同步'))),
          e('div', { className: 'dss-desc' }, '默认手动。自动模式默认每 5 分钟同步，并响应会话活动；更改模式后需重启 DSH。'),
          config.enabled === false ? e('div', { className: 'dss-err' }, '配置文件中 enabled 为 false，自动调度已停用。启用自动模式还需将 enabled 改为 true 并重启。') : null,
          e('label', { className: 'dss-rowline', style: { marginTop: '12px' } },
            ...
            e('span', null, '同步工作区文件')),
          e('div', { className: 'dss-desc' }, '关闭后仍同步会话、附件与设置；不会删除远端已有文件。首次上传前请检查项目中的敏感文件。'),
          e('button', { type: 'submit', className: 'dss-pill', style: { marginTop: '12px' } }, saving ? '保存中…' : '保存偏好')) : null,
        message ? e('div', { className: 'dss-desc', role: 'status' }, message) : null);
    }
    ...
    function Section(props) {
      ...
      const statusDesc = (() => {
        ...
        if (!status.remote) {
          return '尚未连接同步仓库。先在这台电脑运行 gh auth login，再点「立即同步」。\n'
            ...
            + '若只生成本地快照，数据尚未上传。自定义仓库可在 dsh-sync.json 中填写 remote。';
        }
        return status.remote + '\n'
          + '分支 ' + status.branch + ' · ' + (status.auto ? '已配置自动模式（更改模式后需重启）' : '已配置手动模式（更改模式后需重启）')
      ...
      return e('div', { className: 'dss-wrap' },
        e('style', null, STYLE),
        e('div', { className: 'dss-desc' }, '换台电脑，接着做。建议开始前同步一次，结束后再同步一次。 ',
          e('a', { href: 'https://github.com/dpskk2/dsh-sync-plugin/blob/main/docs/getting-started.md', target: '_blank', rel: 'noopener noreferrer' }, '安装与换机指南')),
        /* —— 同步组:状态 + 立即同步 —— */
        e('div', { className: 'dss-sec' },
          ...
          e('div', { className: 'dss-rowline dss-restartrow' },
            e('div', { className: 'dss-rowtext' },
              ...
              e('div', { className: 'dss-desc' }, '高级选项：修复会话分组后尝试重启 DSH，浏览器会短暂断开。依赖本机重启环境；也可以保持关闭，完成同步后手动重启。'),
            ),
            autoRestart === null
          ...
          detail
            ...
            : (status && status.lastOk === false && status.lastMessage
                ...
                : null)),
        e(SyncPreferences, { busy, onSaved: refresh }),
        ...
        /* —— 冲突(「双边保留」)裁决组:呈现给用户抉择 —— */
        e('div', { className: 'dss-sec' },
          e('div', { className: 'dss-h' }, '历史冲突副本'),
          conflicts === null
            ? e('div', { className: 'dss-empty' }, '正在读取冲突…')
            : null,
          conflicts !== null && conflicts.length === 0
            ? e('div', { className: 'dss-desc' }, '当前没有历史冲突副本。新版同步会尝试按文件类型自动合并；普通文件冲突可能保留本机版本。请在同步详情中检查结果及可用备份分支。')
            : null,
          conflicts !== null
