...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function ConflictRow(props) {
      ...
      const [busy, setBusy] = useState(false);
      const label = (c.kind === 'ws' ? '[工作区' + (c.id || '?') + '] ' : '') + c.path;
      const onResolve = (res) => {
        if (busy) return;
    ...
    /* ---------- 归档会话管理处:列出全部会话(含已归档/幽灵),取消归档/彻底删除/回到侧边栏查看 ---------- */
    function fmtDT(ms) {
    ...
    function ArchiveSection() {
      ...
      const act = (id, kind) => {
        ...
        setBusyId(id);
        const p = kind === 'unarchive' ? API.unarchive(id) : kind === 'delete' ? API.delSession(id) : API.unarchive(id);
        p.then((d) => {
          if (d && d.ok) {
            bubble(null, kind === 'unarchive' ? '已取消归档「' + title + '」→ 已回到侧边栏' : kind === 'delete' ? '已彻底删除「' + title + '」' : '已恢复「' + title + '」,请在侧边栏点击查看', true);
          } else { bubble(null, (d && d.error) || '操作失败', false); }
        }).catch((e) => bubble(null, e.message || String(e), false)).finally(() => { setBusyId(null); load(); });
      };
      const archived = (rows || []).filter((r) => r.archived);
      const ghosts = (rows || []).filter((r) => r.ghost);
      const rest = (rows || []).filter((r) => !r.archived && !r.ghost);
      const renderRow = (r) => e('div', { className: 'dss-item', key: r.id },
        e('div', { className: 'dss-main' },
          e('div', { className: 'dss-title', title: r.id + '\n' + r.workspace }, r.title || '(无标题)'),
          e('div', { className: 'dss-meta' },
        ...
        e('button', { type: 'button', className: 'dss-confbtn', disabled: busyId === r.id || r.ghost, onClick: () => act(r.id, 'unarchive') }, busyId === r.id ? '处理中…' : '取消归档'),
        e('button', { type: 'button', className: 'dss-confbtn', disabled: busyId === r.id, onClick: () => act(r.id, 'delete') }, '彻底删除'));
      return e('div', { className: 'dss-wrap' },
        e('div', { className: 'dss-sec' },
          e('div', { className: 'dss-h' }, '归档会话'),
          e('div', { className: 'dss-desc' }, '列出本机全部 DSH 会话(真实文件 + 登记表幽灵)。已归档/幽灵会话默认收起,可「取消归档」让它回到侧边栏,或「彻底删除」。点开后需在侧边栏点击该会话查看原内容。'),
          msg ? e('div', { className: 'dss-err' }, msg) : null,
          rows === null ? e('div', { className: 'dss-empty' }, '正在读取会话…') : null,
          e('div', { className: 'dss-desc' }, '共 ' + ((rows || []).length) + ' 个会话 · 已归档 ' + archived.length + ' · 幽灵 ' + ghosts.length + ' · 活动 ' + rest.length),
          (archived.length ? e('div', null, e('div', { className: 'dss-sub' }, '已归档 / 幽灵(' + (archived.length + ghosts.length) + ')'), archived.concat(ghosts).map(renderRow)) : null),
          (rest.length ? e('div', null, e('div', { className: 'dss-sub' }, '活动(' + rest.length + ')'), rest.map(renderRow)) : null)));
    }
    ...
    function apply(ctx) {
      ...
      // 设置面板分节:归档会话管理处(列出全部会话;取消归档/彻底删除已归档或幽灵会话)
      ctx.slots.inject('settings.section', () => ctx.slots.register({
        name: 'settings.section',
        id: 'dsh-sync-archive',
        order: 410,
