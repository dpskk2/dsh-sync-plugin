  ...
  sessionCwdOf(sdir, group) {
    try {
      const file = fs.existsSync(path.join(sdir, 'session.jsonl'))
        ...
        : fs.existsSync(path.join(sdir, 'session.jsonl.zstd'))
          ? path.join(sdir, 'session.jsonl.zstd')
          : path.join(sdir, 'session.v3.jsonl.zstd');
      ...
      if (fs.existsSync(root)) {
        for (const g of fs.readdirSync(root)) {
          ...
          for (const id of ids) {
            ...
            const hasLog = fs.existsSync(path.join(sdir, 'session.jsonl.zstd')) || fs.existsSync(path.join(sdir, 'session.jsonl')) || fs.existsSync(path.join(sdir, 'session.v3.jsonl.zstd'));
            if (!hasLog) continue;
