...
export class SyncEngine {
  ...
  async backupRemoteHeadWs(gitdir, realPath, remoteHead) {
    ...
    if (!this.ok(cb)) return null;
    await this.gitW(gitdir, realPath, ['push', 'origin', branchName]);
    return branchName;
  }
  ...
  async syncWorkspace(ws, cfg, reason, opts) {
    ...
    let remoteHead = null;
    if (cfg.remote) {
      const rf = await this.gitW(gitdir, realPath, ['fetch', 'origin', '--prune'], 180000);
    ...
    // 远端无该分支 → 首次推送
    if (remoteHead === null) {
      const p = await this.gitW(gitdir, realPath, ['push', '-u', 'origin', branch], 180000);
    ...
    if (localHead === remoteHead) {
      if (committed) {
        const p = await this.gitW(gitdir, realPath, ['push', 'origin', branch], 180000);
    ...
    for (const f of conflicts) await this.gitW(gitdir, realPath, ['checkout', unrelated ? '--theirs' : '--ours', '--', f]);
    ...
    this.log('warn', `[${reason}] 工作区 ${id} 双方都有新提交,自动合并;${conflicts.length} 个冲突文件保留本机版本` + (backupBranch ? `,远端版本备份到 ${backupBranch}` : ''));
    const p = await this.gitW(gitdir, realPath, ['push', 'origin', branch], 180000);
    if (!this.ok(p)) this.log('error', `工作区 ${id} 合并后推送失败: ${p.err.trim().split('\n')[0] || p.code}`);
    return { id, committed, pushed: this.ok(p), pulled: 'merge', conflicts, backupBranch, path: realPath };
  }
  ...
  async syncAllWorkspaces(cfg, reason, opts) {
    ...
    for (const ws of list) {
      let r;
      try { r = await this.syncWorkspace(ws, cfg, reason, opts); }
      catch (e) { r = { id: ws.id, error: String(e?.message || e) }; }
      if (r.error) errors.push({ id: ws.id, title: ws.title, path: ws.path, error: r.error });
      else {
        synced++;
        if (r.pushed) pushed++;
        if (r.pulled) pulled++;
    ...
    return { total: list.length, synced, pushed, pulled, errors };
  }
    ...
    if (cfg.workspaceSync !== false && !this.gitMissing) {
      for (const ws of this.workspaces()) {
        try {
          ...
          if (cfg.remote) this.gitWSync(gitdir, realPath, ['push', 'origin', branch]);
        } catch { /* ignore */ }
      }
    }
  }
