 ...
 *    设置(settings.yaml)同步，专用凭据文件(.credentials.yaml)排除。
...
import { classifyFile, mergeSessionLog, mergeYamlByVmap, mergeJsonByVmap, mergeVmap, diffLeafPaths } from './merge.js';
...
// zstd 解码(读会话头用):Node ≥22.10 才有;旧版本自动降级
...
/**
 * 会话日志文件候选名(按优先级)。DSH 迭代过三代命名:
 *   session.jsonl(明文)→ session.jsonl.zstd(v2 压缩)→ session.v3.jsonl.zstd(v3 迁移后)
 * 登记表自查 / 幽灵清理 / 会话预览 / 冲突预览共用这一份口径 —— 任何一处漏项都会
 * 让「本机明明有这个会话」被误判为幽灵(v0.11.7 修的就是预览侧漏了 v3)。
 */
export const SESSION_LOG_NAMES = ['session.jsonl', 'session.jsonl.zstd', 'session.v3.jsonl.zstd'];
...
export const DEFAULT_CONFIG = {
  ...
  workspaceExtraIgnore: ['node_modules', '.pnpm-store', '.npm-cache'], // 工作区内容额外排除(目录名;.git 恒排除);写入影子仓库 info/exclude,不碰真实文件夹
  autoRestartAfterRepair: false, // 同步后自查把会话补进登记表(修复「未分组」)后,自动重启 dsh 让新实例重建索引并加载修复版。
  //  根因(2026-09-11 实证):侧栏「未分组」由运行中 dsh 实例渲染,插件修好的磁盘 workspace.json 会被运行实例的
  //  mutate() 主动修剪覆盖;只有重启 dsh(重启前不做任何侧栏操作)才生效。开启后同步若补登记有变化会自动重启,
  //  浏览器会话会短暂断开、刷新恢复;不需要自动重启时保持 false,可手动跑 ~/.dsh/fix-ungrouped-oneshot.mjs。
};
...
const BUILTIN_IGNORE = [
  ...
  '.dshw-usage.json',
  '',
  '# 可再生状态 —— 每台机器每次运行都重写(缓存/诊断日志/插件市场状态),不同步,避免每次合并必冲突',
  'cache/',
  'logs/',
  ...
  '.dsh-sync.state.json',
  '',
  '# 工作区路径覆盖(「换位置」的本机专属设置:机器 A 的覆盖不能决定机器 B 的工作区位置)',
  'storages/workspace-local-paths.json',
  '',
...
/**
 * 需要确保出现在 .gitignore 里的条目(密钥排除 + 依赖存储排除 + 本机专属路径覆盖);
 * 对已存在的 .gitignore 由 ensureRepo 补写,对旧版本被移除的条目自动恢复。
 */
const BUILTIN_IGNORE_ENSURE = ['.credentials.yaml', '.pnpm-store/', 'storages/workspace-local-paths.json', 'cache/', 'logs/', 'profiles/web/.dsh-market/', '**/*.log'];
...
export class SyncEngine {
  ...
  constructor(home, overrides = {}, log = () => {}) {
    ...
    this._verifiedPrivateRemote = null;
    /** 会话 cwd 解析缓存(key = 会话文件路径|size|mtime),自查每轮跑两次但不重复解析 */
    this._cwdCache = new Map();
    /** 孤儿工作区影子仓库自查节流(24h 一次) */
    this._orphanCheckedAt = 0;
    /** 待裁决冲突扫描缓存({ at, list }),同一轮同步内多处调用只扫一次 */
    this._conflictsCache = null;
    /** 工作区影子仓库配置已应用标记(gitdir → 期望值指纹),避免每轮重复 config 读写 */
    this._mirrorApplied = new Map();
  ...
  /** 主仓库对象统计一次(loose 对象数/体积 + pack 体积);失败返回 null。
   *  仓库体积展示与 gc 策略共用,避免同一次同步起两次 git count-objects。 */
  repoStats() {
    try {
      const r = spawnSync('git', ['count-objects', '-vH'], { cwd: this.home, windowsHide: true, encoding: 'utf8', timeout: 15000 });
      const out = r.stdout || '';
      const looseCount = Number((/^count:\s*(\d+)/m.exec(out) || [])[1] ?? 0);
      const sizeM = /^size:\s*([\d.]+)\s*(\S+)/m.exec(out);
      const packM = /size-pack:\s*([\d.]+)\s*(\S+)/.exec(out);
      const looseBytes = sizeM ? parseBytes(sizeM[1], sizeM[2]) : 0;
      return {
        looseCount,
        looseBytes,
        packBytes: packM ? parseBytes(packM[1], packM[2]) : null,
      };
    } catch { return null; }
  }
  ...
  /** Remote writes are allowed only after GitHub confirms private visibility. */
  async verifyPrivateRemote(remote) {
    const url = String(remote || '').trim();
    const match = /^(?:https:\/\/github\.com\/|git@github\.com:|ssh:\/\/git@github\.com\/)([\w.-]+)\/([\w.-]+?)(?:\.git)?\/?$/i.exec(url);
    if (!match) return { error: '仅支持不含凭据的 GitHub 仓库地址（HTTPS 或 SSH）；请检查 remote。' };
    const result = await this.gh(['repo', 'view', `${match[1]}/${match[2]}`, '--json', 'visibility']);
    if (!this.ok(result)) return { error: '无法确认 GitHub 仓库是否私有。请执行 gh auth login 并检查仓库访问权限后重试；本次不会上传。' };
    try {
      if (JSON.parse(result.out).visibility === 'PRIVATE') return { ok: true };
    } catch { /* invalid response must fail closed */ }
    return { error: 'GitHub 仓库不是私有仓库，或无法确认可见性；本次不会上传。请改用私有仓库。' };
  }
    ...
    if (this.ok(view)) {
      ...
      try {
        ...
        if (parsed.visibility !== 'PRIVATE') {
          this._bootstrapError = `${owner}/${repo} 不是已确认的私有仓库；请改用私有仓库。本次不会上传。`;
          this.log('error', this._bootstrapError);
          return null;
        }
      } catch {
        this._bootstrapError = `无法确认 ${owner}/${repo} 的私有状态；本次不会上传。`;
        this.log('error', this._bootstrapError);
        return null;
      }
  ...
  async ensureRepo(cfg) {
    ...
    try {
      const targets = ['.credentials.yaml', '.dshw-usage.json', '.dshw-size.json', '.anonymous-user-id', '.pnpm-store', 'storages/workspace-local-paths.json', 'cache', 'logs', 'profiles/web/.dsh-market'];
  ...
  async commitIfDirty(cfg, force = false) {
    ...
    // 提交前维护字段级版本图(settings.yaml/package.json 的 LWW 时钟),让「两机改同一字段」可收敛
    try { await this.updateCrdtVmaps(); } catch (e) { this.log('warn', '版本图维护异常(不影响同步): ' + String(e?.message || e)); }
  ...
  /** 回收仓库空间。触发条件不只看提交次数:DSH 每次快照都写入新 blob,
   *  loose 对象会快速堆积(实测 238MB 仓库里 142MB 是未打包对象),
   *  因此还按 loose 体积阈值触发,避免只在「本进程满 20 次提交」时才回收。 */
  async gcIfNeeded() {
    const stats = this.repoStats();
    const looseBytes = stats ? stats.looseBytes : 0;
    const looseCount = stats ? stats.looseCount : 0;
    const byVolume = looseBytes >= 64 * 1024 * 1024 || looseCount >= 5000 || this.commitsSinceGc >= 20;
    if (!byVolume) return;
    this.commitsSinceGc = 0;
    ...
    else this.log('info', `已执行 git gc 回收仓库空间(回收前 loose ${humanSize(looseBytes)}/${looseCount} 个对象)`);
  }
  ...
  async backupRemoteHead(gitdir, realPath, remoteHead) {
    ...
    const branchName = `backup/${ts()}-${randomUUID()}`;
    ...
    if (!this.ok(pb)) {
      ...
      return { branch: branchName, pushed: false };
    }
   ...
   * 任何残留都意味着分层合并写入失败——此时绝不能 add/commit,否则把标记当内容
  ...
  /** 主仓库里已被跟踪的历史冲突拷贝(.dsh-conflict-<ts>)。
   *  只查主仓库一次 ls-files,不遍历各工作区 —— pruneCacheConflicts 每轮同步都要用,
   *  走 outstandingConflicts 会给每个工作区起 2 次 git 子进程(实测 181ms/次)。 */
  mainConflictCopies() {
    const out = [];
    const r = this.gitSync(['ls-files', '--', ':(glob)**/*.dsh-conflict-*']);
    if (!this.ok(r)) return out;
    for (const line of String(r.out).split('\n')) {
      const t = line.trim();
      if (!t) continue;
      const m = /^(.*)\.dsh-conflict-(.+)$/.exec(t);
      if (m) out.push({ kind: 'main', id: null, path: m[1], copy: m[0], conflictAt: m[2] });
    }
    return out;
  }

  /** 扫描主仓库 + 各工作区影子仓库,列出历史遗留的冲突拷贝(.dsh-conflict-<ts>)。
   *  新版分层合并引擎不再生成冲突拷贝,此方法仅用于迁移清理与只读审计。
   *  每次调用要给每个仓库起一次 git 子进程(4 工作区约 180ms),而同一轮同步里
   *  设置页/状态接口/工具常连续调用多次 → 结果缓存 15s,并在每轮同步结束时失效。
   *  (工作区冲突拷贝不在主仓库里,所以不能只扫主仓库。) */
  outstandingConflicts() {
    const now = Date.now();
    if (this._conflictsCache && now - this._conflictsCache.at < 15000) return this._conflictsCache.list;
    ...
    this._conflictsCache = { at: now, list: out };
    return out;
  }

  /* ================= 分层合并引擎(替换「双边保留 + 人工裁决」) ================= */

  /** crdt 文件的版本图 sidecar 路径(storages/sync-meta/<relpath>.vmap.json) */
  vmapPathOf(relPath) {
    return 'storages/sync-meta/' + String(relPath).replace(/\\/g, '/') + '.vmap.json';
  }

  /** 读 git 冲突三态 blob(:1 base / :2 ours / :3 theirs),主仓库与工作区影子仓库通用 */
  async conflictBlobs(gitdir, base, f) {
    const call = (a) => (gitdir ? this.gitBytesW(gitdir, base, a) : this.gitBytes(a));
    const read = async (stage) => {
      const r = await call(['show', stage + f]);
      return (this.ok(r) && r.out && r.out.length) ? r.out : null;
    };
    return { base: await read(':1:'), ours: await read(':2:'), theirs: await read(':3:') };
  }

  /** 字段级 CRDT 合并(crdt-json / crdt-yaml / crdt-meta)。成功返回 true 并已写回工作树+add。 */
  async mergeCrdtFile(f, type) {
    const vmapPath = this.vmapPathOf(f);
    const [b, vmB] = await Promise.all([
      this.conflictBlobs(null, this.home, f),
      this.conflictBlobs(null, this.home, vmapPath),
    ]);
    if (!b.ours || !b.theirs) return false;
    const text = (x) => (x ? x.toString('utf8') : null);
    const parseJson = (x) => { try { return x ? JSON.parse(text(x)) : undefined; } catch { return undefined; } };
    const vmapOf = (x) => { const v = parseJson(x); return (v && typeof v === 'object') ? v : {}; };

    if (type === 'crdt-meta') {
      const mergedVmap = mergeVmap(vmapOf(vmB.ours), vmapOf(vmB.theirs));
      this._writeMerged(f, Buffer.from(JSON.stringify(mergedVmap, null, 2) + '\n', 'utf8'));
      await this.git(['add', '--', f]);
      return true;
    }

    const oursVmap = vmapOf(vmB.ours);
    const theirsVmap = vmapOf(vmB.theirs);
    const mergedVmap = mergeVmap(oursVmap, theirsVmap);
    let outText = null;
    if (type === 'crdt-yaml') {
      const r = mergeYamlByVmap(text(b.base), text(b.ours), text(b.theirs), oursVmap, theirsVmap);
      if (!r.ok) return false;
      outText = r.value;
    } else {
      const bObj = parseJson(b.base), oObj = parseJson(b.ours), tObj = parseJson(b.theirs);
      if (oObj === undefined || tObj === undefined) return false;
      outText = JSON.stringify(mergeJsonByVmap(bObj, oObj, tObj, oursVmap, theirsVmap, ''), null, 2) + '\n';
    }
    this._writeMerged(f, Buffer.from(outText, 'utf8'));
    await this.git(['add', '--', f]);
    // 写回合并后的版本图(仅当有内容或任一侧存在版本图,避免引导期造空文件)
    if (Object.keys(mergedVmap.fields || {}).length || vmB.base || vmB.ours || vmB.theirs) {
      this._writeMerged(vmapPath, Buffer.from(JSON.stringify(mergedVmap, null, 2) + '\n', 'utf8'));
      await this.git(['add', '--', vmapPath]);
    }
    return true;
  }

  /** 写合并结果到工作树(目录自动创建;主仓库用) */
  _writeMerged(relPath, buf) {
    const target = path.join(this.home, relPath);
    fs.mkdirSync(path.dirname(target), { recursive: true });
    fs.writeFileSync(target, buf);
  }

  /* ================= 提交时版本图(vmap)维护 —— 字段级 LWW 的时钟来源 ================= */

  /** 本机 CRDT actor id(UUID,持久化在 .dsh-sync.state.json,机器本地不上云) */
  actorId() {
    const sp = path.join(this.home, '.dsh-sync.state.json');
    let state = {};
    try { state = JSON.parse(fs.readFileSync(sp, 'utf8')) || {}; } catch { /* 缺省 */ }
    if (!state.crdtActor) {
      state.crdtActor = randomUUID();
      try { fs.writeFileSync(sp, JSON.stringify(state, null, 2) + '\n', 'utf8'); } catch { /* ignore */ }
    }
    return state.crdtActor;
  }

  /** 读 HEAD 版本的文件文本(无则 null) */
  async _headTextOf(f) {
    const r = await this.git(['show', 'HEAD:' + f]);
    return (this.ok(r) && r.out) ? r.out : null;
  }

  /** 读工作树文件文本(不存在返回 null) */
  _workTextOf(f) {
    const p = path.join(this.home, f);
    return fs.existsSync(p) ? fs.readFileSync(p, 'utf8') : null;
  }

  /** 读某 crdt 文件当前的 vmap(工作树优先,否则 HEAD;都无返回 {}) */
  async _readVmap(vmapPath) {
    const wt = path.join(this.home, vmapPath);
    if (fs.existsSync(wt)) { try { const v = JSON.parse(fs.readFileSync(wt, 'utf8')); return (v && typeof v === 'object') ? v : {}; } catch {} }
    const r = await this.git(['show', 'HEAD:' + vmapPath]);
    if (this.ok(r) && r.out) { try { const v = JSON.parse(r.out); return (v && typeof v === 'object') ? v : {}; } catch {} }
    return {};
  }

  /**
   * 提交前给发生字段级变化的 crdt 文件(settings.yaml / package.json)更新版本图:
   * diff 工作树 vs HEAD → 变更叶子打上新 Lamport 时钟 { t: maxT+1, actor }。
   * 这保证「两机改同一字段」能按「后写胜」收敛(见 docs/merge-engine-refactor.md §5)。
   */
  async updateCrdtVmaps() {
    const st = await this.git(['status', '--porcelain']);
    ...
    for (const line of String(st.out).split('\n')) {
      ...
      if ((t === 'crdt-json' || t === 'crdt-yaml') && p !== 'storages/workspace.json' && p !== '.credentials.yaml') changed.add(p);
    }
    ...
    for (const f of changed) {
      try {
        const vmapPath = this.vmapPathOf(f);
        const [headText, workText, oldVmap] = await Promise.all([
          this._headTextOf(f), this._workTextOf(f), this._readVmap(vmapPath),
        ]);
        if (workText == null) continue;
        let baseObj, newObj;
        if (classifyFile(f) === 'crdt-yaml') {
          const pn = parseYaml(workText); if (!pn.ok) continue;
          newObj = pn.value;
          const pb = headText != null ? parseYaml(headText) : { ok: true, value: undefined };
          if (!pb.ok) continue;
          baseObj = pb.value;
        } else {
          try { newObj = JSON.parse(workText); } catch { continue; }
          try { baseObj = headText != null ? JSON.parse(headText) : undefined; } catch { baseObj = undefined; }
        }
        const changedPaths = diffLeafPaths(baseObj, newObj);
        if (!changedPaths.size) continue;   // 仅键序/无实际字段变化 → 不 bump
        const fields = { ...(oldVmap.fields || {}) };
        let maxT = 0;
        for (const c of Object.values(fields)) if (c && typeof c.t === 'number' && c.t > maxT) maxT = c.t;
        const t = maxT + 1;
        for (const p of changedPaths) fields[p] = { t, actor };
        this._writeMerged(vmapPath, Buffer.from(JSON.stringify({ format: 1, path: f, fields }, null, 2) + '\n', 'utf8'));
      } catch (e) {
        this.log('warn', `版本图更新失败(不影响同步): ${f}: ${String(e?.message || e)}`);
      }
    }
  }
  ...
  async ensureWorkspaceMirror(id, realPath, cfg) {
    ...
    // 确保身份/worktree/origin(路径解析可能随机器/配置变化)。
    // 一次 config --get-regexp 读出全部相关键(原来 7~8 次 spawn,每轮同步 × 每个工作区),
    // 与期望值比对后只写差异项;同一进程内已按 (gitdir|worktree|remote|branch) 应用过则跳过。
    const appliedKey = [gitdir, realPath, cfg.remote || '', branch].join('|');
    if (this._mirrorApplied?.get(gitdir) !== appliedKey) {
      const want = {
        'core.bare': 'false',
        'core.worktree': realPath,
        'core.autocrlf': 'false',
        'core.safecrlf': 'false',
      ...
      const cur = await this.gitW(gitdir, realPath, ['config', '--get-regexp', '^(core\\.|user\\.|commit\\.|remote\\.origin\\.)']);
      const got = new Map();
      if (this.ok(cur)) {
        for (const line of String(cur.out).split('\n')) {
          const sp = line.indexOf(' ');
          if (sp > 0) got.set(line.slice(0, sp), line.slice(sp + 1).trim());
        }
      }
      for (const [k, v] of Object.entries(want)) {
        if (got.get(k) !== v) await this.gitW(gitdir, realPath, ['config', k, v]);
      }
      if (cfg.remote) {
        const wantUrl = String(cfg.remote).replace(/\/+$/, '');
        if (!got.has('remote.origin.url')) await this.gitW(gitdir, realPath, ['remote', 'add', 'origin', cfg.remote]);
        else if (got.get('remote.origin.url').replace(/\/+$/, '') !== wantUrl) await this.gitW(gitdir, realPath, ['remote', 'set-url', 'origin', cfg.remote]);
        // 只取本工作区自己的分支:不再用默认 refspec 拉取全部远端分支(主仓库 main 的会话历史
        // 每个工作区都拉一遍,既慢又让工作区仓库膨胀到几十 MB)
        const fetchSpec = '+refs/heads/' + branch + ':refs/remotes/origin/' + branch;
        if (got.get('remote.origin.fetch') !== fetchSpec) await this.gitW(gitdir, realPath, ['config', 'remote.origin.fetch', fetchSpec]);
      }
      this._mirrorApplied.set(gitdir, appliedKey);
    }
  ...
  /**
   * 孤儿工作区影子仓库自查:登记表里已不存在的工作区(工作区被删除 / 并集去重换了 id),
   * 其 workspace-repos/<id>.git 会永久残留 —— 实测本机 156MB 里 153MB 是孤儿。
   * 这里只**报告**(不自动删:那是用户数据,删除应由用户决定),每 24h 最多一次。
   */
  reportOrphanWorkspaceRepos(cfg) {
    const now = Date.now();
    if (now - (this._orphanCheckedAt || 0) < 24 * 3600 * 1000) return null;
    this._orphanCheckedAt = now;
    if (cfg.workspaceSync === false) return null;
    const dir = path.join(this.home, 'workspace-repos');
    let entries = [];
    try { entries = fs.readdirSync(dir).filter((n) => n.endsWith('.git')); } catch { return null; }
    if (!entries.length) return null;
    const live = new Set(this.workspaces().map((w) => w.id + '.git'));
    const sizeOf = (p) => {
      let total = 0;
      const walk = (q) => {
        let st = null;
        ...
        let names = [];
        try { names = fs.readdirSync(q); } catch { return; }
        for (const n of names) walk(path.join(q, n));
      };
      walk(p);
      return total;
    };
    const orphans = entries.filter((n) => !live.has(n));
    if (!orphans.length) return null;
    let bytes = 0;
    for (const n of orphans) bytes += sizeOf(path.join(dir, n));
    this.log('info', `工作区影子仓库:${orphans.length} 个已不在登记表中(共 ${humanSize(bytes)},占 ${path.join(this.home, 'workspace-repos')});确认不再需要可手工删除这些 <工作区id>.git 目录回收空间`);
  ...
  async syncWorkspace(ws, cfg, reason, opts) {
    ...
    if (cfg.remote) {
      ...
      if (this.ok(rf)) remoteHead = await this.rev('refs/remotes/origin/' + branch, gitdir, realPath);
        ...
        // An exact fetch refspec also fails for a genuinely new workspace branch.
        // Only a successful remote query proving absence permits first upload.
        const probe = await this.gitW(gitdir, realPath, ['ls-remote', '--heads', 'origin', 'refs/heads/' + branch]);
        fetchFailed = !this.ok(probe) || !!probe.out.trim();
        if (fetchFailed) this.log('warn', `工作区 ${id} fetch 失败(远端不可达?仅本地快照): ${rf.err.trim().split('\n')[0] || rf.code}`);
      }
    }
    // 快路径:本地与远端同头 + 工作树干净 → 本轮没有任何可提交/可推送的东西。
    // 常见情况(两台电脑都空闲时点同步)下省掉 commit/add/status 等 4~5 次 git 子进程。
    // forceCommit 只表示「不受提交节流限制」(手动触发),不代表一定有变更,故不排除此路径;
    // 无 remote(仅本地快照)不走此路径。工作树脏、或影子仓库缺 core.worktree 时也不会命中。
    if (cfg.remote && remoteHead !== null) {
      const localHead0 = await this.rev('HEAD', gitdir, realPath);
      if (localHead0 === remoteHead) {
        const stClean = await this.gitW(gitdir, realPath, ['status', '--porcelain']);
        if (this.ok(stClean) && !String(stClean.out).trim()) {
    ...
    if (fetchFailed) return { id, committed, pushed: false, pulled: false, error: 'fetch', path: realPath };
    ...
    if (localHead === remoteHead) {
      if (committed) {
        ...
        return { id, committed, pushed: this.ok(p), ...(!this.ok(p) && { error: 'push' }), path: realPath };
      }
      return { id, committed: false, pushed: false, pulled: false, path: realPath };
    }
    // 节流攒下未提交变更 → 强制提交
    ...
    if (this.ok(mg)) {
      ...
      return { id, committed, pushed: this.ok(p), pulled: 'merge', unrelated, ...(!this.ok(p) && { error: 'push' }), path: realPath };
    }
    const cf = await this.gitW(gitdir, realPath, ['diff', '--name-only', '--diff-filter=U']);
    const conflicts = this.ok(cf) ? cf.out.trim().split('\n').filter(Boolean) : [];
    const backup = unrelated ? null : await this.backupRemoteHead(gitdir, realPath, remoteHead);
    const backupBranch = backup?.branch || null;
    if (unrelated) {
      for (const f of conflicts) await this.gitW(gitdir, realPath, ['checkout', '--theirs', '--', f]);
    } else {
      // 工作区内容全是 opaque 文件:真冲突时确定性取本机,远端版本在 backup 分支 + origin 历史(零人工裁决)
      for (const f of conflicts) {
        if (!f || f.includes('\0')) continue;
        await this.gitW(gitdir, realPath, ['checkout', '--ours', '--', f]);
      }
    }
    ...
    this.log('info', `[${reason}] 工作区 ${id} 双方都有新提交,自动合并;${conflicts.length} 个冲突文件已自动取本机(远端在 backup 分支/origin 历史)` + (backupBranch ? `,远端头备份到 ${backupBranch}` : ''));
    const p = await this.runWithRetry('push', '工作区 ' + (label || id), ['push', 'origin', branch], { gitdir, realPath, timeoutMs: 180000, onProgress: (d) => this.liveTransfer('push', '工作区 ' + (label || id), d) });
    if (!this.ok(p)) this.log('error', `工作区 ${id} 合并后推送失败: ${p.err.trim().split('\n')[0] || p.code}`);
    return {
      id, committed, pushed: this.ok(p), pulled: 'merge',
      conflicts, conflictCopies: [], backupBranch, backupPushed: backup?.pushed ?? false, ...(!this.ok(p) && { error: 'push' }), path: realPath,
    };
  }
  ...
  async syncAllWorkspaces(cfg, reason, opts) {
    ...
    for (const r of results) {
      if (r.backupBranch) backups.push({ id: r.id, branch: r.backupBranch, pushed: r.backupPushed === true });
    ...
    return { total: list.length, synced, pushed, pulled, errors, conflictCopies, created, backups };
  }

  /** 主数据同步(会话/settings/插件/技能/工作区元数据);工作区文件夹内容由外层 _syncOnce 追加处理 */
  /**
   * 枚举本机全部会话目录:返回 [{ id, group, file, size, mtimeMs }]。
   * 会话文件候选名以 SESSION_LOG_NAMES 为准(v0.11.7 前 index.js 的预览漏了 v3,
   * 收敛到一处常量即可根除同类问题);登记表自查/幽灵清理/预览共用同一份口径。
   */
  scanSessions() {
    ...
    for (const group of groups) {
      ...
      for (const id of ids) {
        ...
        const sdir = path.join(root, group, id);
        let file = null, stat = null;
        for (const name of SESSION_LOG_NAMES) {
          const p = path.join(sdir, name);
          try { const st = fs.statSync(p); if (st.isFile()) { file = p; stat = st; break; } } catch { /* 试下一个候选名 */ }
        }
        if (!file) continue;
        out.push({ id, group, file, size: stat.size, mtimeMs: stat.mtimeMs });
      }
    }
    return out;
  }
   ...
   *  sdir 也可直接传会话文件路径(预览/冲突拷贝等非目录场景)。 */
  sessionCwdOf(sdir, group) {
    try {
      let file;
      try {
        if (fs.statSync(sdir).isFile()) file = sdir;
      } catch { /* 不存在则按目录处理 */ }
      if (!file) {
        for (const name of SESSION_LOG_NAMES) {
          const p = path.join(sdir, name);
      ...
      if (!file) return decodeGroup(group);
      const fd = fs.openSync(file, 'r');
      let text = '';
      try {
        // 会话头在第一个 zstd 帧里,zstd 帧头自带大小;先读 1MB(足够容纳大 header),
        // 只在没读到 type session 时再补读一次,避免每个会话固定读 2MB。
        const readHead = (size) => {
          const head = Buffer.alloc(size);
          const n = fs.readSync(fd, head, 0, head.length, 0);
          const buf = head.subarray(0, n);
          if (buf.subarray(0, 4).equals(ZSTD_MAGIC) && typeof zstdDecompressSync === 'function') {
            const i2 = buf.indexOf(ZSTD_MAGIC, 4);
            const frame = i2 === -1 ? buf : buf.subarray(0, i2);
            try { return zstdDecompressSync(frame).toString('utf8'); } catch { return ''; }
          }
        ...
        text = readHead(1024 * 1024);
        let cwd = parseCwd(text);
        if (!cwd) {
          text = readHead(2 * 1024 * 1024);
          cwd = parseCwd(text);
        }
        if (cwd) return cwd;
   ...
   *
   * 性能:本函数每轮同步会跑两次(同步前 + 同步后自查),而稳态下会话基本都已登记。
   * 因此先按登记表算出「已登记会话集合」,只对**未登记**的会话解析日志头取 cwd
   * (实测 22 会话 30.5ms → 2.3ms,结果完全一致);解析结果再按
   * (文件, size, mtime) 缓存,跨轮复用且日志变化时自动失效。
   */
  repairWorkspaceRegistry() {
    try {
      ...
      // 归一化:归档会话若仍残留在工作区 sessionIds(旧版 repair 误塞回的双重登记),从工作区移除(归档优先)
      const archivedSet = new Set(Array.isArray(global_.archivedSessionIds) ? global_.archivedSessionIds : []);
      let unarchived = 0;
      if (archivedSet.size) {
        for (const w of Object.values(workspaces)) {
          if (w && Array.isArray(w.sessionIds)) {
            const before = w.sessionIds.length;
            w.sessionIds = w.sessionIds.filter((sid) => !archivedSet.has(sid));
            unarchived += before - w.sessionIds.length;
          }
        }
      }
      // 已登记会话(去重后重新计算,因为去重会把被丢弃记录的会话并进保留记录)
      // 归档会话也算「已登记」——否则 repair 会把用户归档的会话又塞回工作区(双重登记 / 归档被撤销)
      const registered = new Set();
      for (const w of Object.values(workspaces)) {
        if (w && Array.isArray(w.sessionIds)) for (const sid of w.sessionIds) registered.add(sid);
      }
      if (Array.isArray(global_.archivedSessionIds)) for (const sid of global_.archivedSessionIds) registered.add(sid);
      let repaired = 0, created = 0;
      for (const s of this.scanSessions()) {
        if (registered.has(s.id)) continue; // 已登记 → 无需解析会话头(稳态下几乎所有会话都走这里)
        const cacheKey = s.file + '|' + s.size + '|' + s.mtimeMs;
        let cwd = this._cwdCache.get(cacheKey);
        if (cwd === undefined) {
          cwd = this.sessionCwdOf(path.dirname(s.file), s.group);
          this._cwdCache.set(cacheKey, cwd);
          if (this._cwdCache.size > 2000) this._cwdCache.clear(); // 防止长期运行无限增长
        }
        if (!cwd) continue;
        ...
        if (!wid) {
          ...
          workspaces[wid] = { path: cwd, title: titleFromPath(cwd), sessionIds: [s.id], createdAt: now, updatedAt: now };
        ...
        if (!list.includes(s.id)) { list.unshift(s.id); registered.add(s.id); repaired++; }
      }
      if (repaired || created || deduped || unarchived) {
        fs.writeFileSync(wsFile, JSON.stringify(ws, null, 2));
        this.log('info', `工作区登记表修复:${repaired} 个会话补进对应工作区,新建 ${created} 个工作区条目,去重 ${deduped} 个重复工作区,${unarchived} 个归档会话从工作区移除(随本次快照提交同步;若侧栏会话仍在「未分组」,请重启 dsh 以重新加载登记表)`);
      }
      return { ok: true, repaired, created, deduped, unarchived };
    } catch (e) {
  ...
  async unionMergeWorkspaceJson() {
    try {
      ...
      const parse = (r) => { try { return this.ok(r) && r.out ? JSON.parse(r.out) : null; } catch { return null; } };
      ...
      try {
        const cfg = this.loadConfig();
        const localFiles = this.localSessionIds();
  ...
  /** 本机会话 id 集合(sessions/ 下真实存在的会话目录;登记表自查/幽灵清理/并集合并共用) */
  localSessionIds() {
    const set = new Set();
    for (const s of this.scanSessions()) set.add(s.id);
    return set;
  }

  /** 工作区登记表里已登记的会话 id 集合(各工作区 sessionIds + archivedSessionIds 的并集)。
   *  用于比较「同步前后登记是否变化」——有会话被新登记(或取消登记)就说明运行中 dsh 的内存索引可能已过期。 */
  registeredSessionIds() {
    const set = new Set();
    try {
      const ws = JSON.parse(fs.readFileSync(path.join(this.home, 'storages', 'workspace.json'), 'utf8'));
      if (Array.isArray(ws.global?.archivedSessionIds)) for (const s of ws.global.archivedSessionIds) set.add(s);
      for (const w of Object.values(ws.tables?.workspaces ?? {})) {
        if (w && Array.isArray(w.sessionIds)) for (const s of w.sessionIds) set.add(s);
      }
    } catch { /* 缺省 */ }
    return set;
  }
  ...
  async pruneCacheConflicts() {
    try {
      const cfg = this.loadConfig();
      const list = this.mainConflictCopies().filter((c) => /^storages\/session_projcache\//i.test(c.path || ''));
  ...
  /** P5 迁移:一次性清理历史 .dsh-conflict-<ts> 拷贝(内容本就在 git 历史里,删除不丢数据)。
   *  用 .dsh-sync.state.json 的 legacyConflictCleanup 标记保证只跑一次;仅主仓库,工作区遗留项由 sync_conflicts 只读列出。 */
  async migrateLegacyConflictCopies() {
    const sp = path.join(this.home, '.dsh-sync.state.json');
    let state = {};
    try { state = JSON.parse(fs.readFileSync(sp, 'utf8')) || {}; } catch { /* 缺省 */ }
    if (state.legacyConflictCleanup) return { ok: true, removed: 0, skipped: true };
    try {
      const cfg = this.loadConfig();
      const list = this.mainConflictCopies().filter((c) => !/^storages\/session_projcache\//i.test(c.path || ''));
      if (!list.length) { state.legacyConflictCleanup = true; }
      else {
        for (const c of list) {
          const copyAbs = path.join(this.home, c.copy);
          if (fs.existsSync(copyAbs)) fs.rmSync(copyAbs, { force: true });
        }
        state.legacyConflictCleanup = true;
        const st = await this.git(['status', '--porcelain']);
        if (this.ok(st) && String(st.out).trim()) {
          const added = await this.git(['add', '-A']);
          if (this.ok(added)) {
            const cm = await this.git(['commit', '-m', 'dsh-sync-plugin: 清理历史冲突拷贝(新版已自动合并冲突,内容保留在 git 历史)']);
            if (this.ok(cm) && cfg.remote) await this.git(['push', 'origin', cfg.branch]); // 推送失败不阻塞
          }
        }
      }
      try { fs.writeFileSync(sp, JSON.stringify(state, null, 2) + '\n', 'utf8'); } catch { /* ignore */ }
      ...
      this.log('warn', '历史冲突拷贝清理失败(不影响同步): ' + String(e?.message || e));
  ...
  async _mainSync(reason, opts = {}) {
    ...
    if (!cfg.remote && cfg.autoRepo !== false) {
      ...
      if (this._bootstrapError) return { error: this._bootstrapError };
    }

    this._verifiedPrivateRemote = null;
    if (cfg.remote) {
      const visibility = await this.verifyPrivateRemote(cfg.remote);
      if (!visibility.ok) return { error: visibility.error };
      this._verifiedPrivateRemote = cfg.remote;
    }

    if (!(await this.ensureRepo(cfg))) return { error: 'ensure-repo' };

    // P5 迁移:一次性清理历史 .dsh-conflict 拷贝(内容在 git 历史中,不丢数据;仅跑一次)
    try { await this.migrateLegacyConflictCopies(); } catch (e) { this.log('warn', '历史冲突拷贝迁移异常: ' + String(e?.message || e)); }
    ...
    if (localHead === remoteHead) {
      if (committed) {
        ...
        return { committed, pushed: this.ok(p), ...(!this.ok(p) && { error: 'push' }) };
      }
    ...
    if (this.ok(mg)) {
      ...
      return { committed, pushed: this.ok(p), pulled: 'merge', unrelated, ...(!this.ok(p) && { error: 'push' }) };
    }
    const cf = await this.git(['diff', '--name-only', '--diff-filter=U']);
    let conflicts = this.ok(cf) ? cf.out.trim().split('\n').filter(Boolean) : [];
    const backup = unrelated ? null : await this.backupRemoteHead(null, null, remoteHead);
    const backupBranch = backup?.branch || null;
    if (unrelated) {
      // 无共同历史(新机并入远端):同名文件以远端为准,本地独有文件保留
      for (const f of conflicts) await this.git(['checkout', '--theirs', '--', f]);
    } else {
      // 分层合并引擎:按文件类型确定性合并,任何类型都不再产出「待人工裁决」的 .dsh-conflict 拷贝
      let autoMerged = 0;
      let keptLocal = 0;
      for (const f of conflicts) {
        if (!f || f.includes('\0')) continue;
        try {
          if (f === 'storages/workspace.json') {
            // 工作区登记:并集合并(双方工作区/会话映射都保留);失败自动取本机(远端登记在历史可找回)
            if (await this.unionMergeWorkspaceJson()) {
              await this.git(['add', '--', f]);
              autoMerged += 1;
              this.log('info', `[${reason}] workspace.json 已并集合并`);
            } else {
          ...
          if (type === 'session-log') {
            const b = await this.conflictBlobs(null, this.home, f);
            if (b.ours && b.theirs) {
              const r = mergeSessionLog(b.ours, b.theirs);
              if (r.ok) {
                this._writeMerged(f, r.value);
                await this.git(['add', '--', f]);
                autoMerged += 1;
                this.log('info', `[${reason}] 会话日志已自动并集合并: ${f}`);
                continue;
              }
            }
            throw new Error('session-log merge failed');
          }
          if (type === 'crdt-json' || type === 'crdt-yaml') {
            if (f === '.credentials.yaml') { await this.git(['checkout', '--ours', '--', f]); keptLocal += 1; continue; }
            if (await this.mergeCrdtFile(f, type)) {
              autoMerged += 1;
              this.log('info', `[${reason}] ${type === 'crdt-yaml' ? 'YAML' : 'JSON'} 配置已字段级合并: ${f}`);
              continue;
            }
            throw new Error(type + ' merge failed');
          }
          if (type === 'crdt-meta') {
            if (await this.mergeCrdtFile(f, type)) { autoMerged += 1; this.log('info', `[${reason}] vmap 已合并: ${f}`); continue; }
            throw new Error('vmap merge failed');
          }
          // opaque(session_projcache/.gitignore/工作区文件等)→ 本机优先,远端在 backup 分支 + origin 历史
          await this.git(['checkout', '--ours', '--', f]);
          keptLocal += 1;
        } catch (e) {
          try { await this.git(['checkout', '--ours', '--', f]); } catch {}
          keptLocal += 1;
          this.log('warn', `[${reason}] ${f} 合并失败,自动取本机(远端见 backup 分支/origin 历史): ${String(e?.message || e)}`);
        }
      }
      if (autoMerged) this.log('info', `[${reason}] 分层合并:${autoMerged} 个文件已自动合并`);
      if (keptLocal) this.log('info', `[${reason}] 分层合并:${keptLocal} 个 opaque 文件自动取本机`);
    }
    // 守卫:任何冲突文件仍残留合并标记 → 中止合并,绝不把标记提交/推送(标记会让页面/文档打开为空白或乱码)
    ...
    if (unrelated) {
      ...
      this.log(
        'info',
        `[${reason}] 双方都有新提交,已分层自动合并;${conflicts.length} 个冲突文件按类型确定性合并(零人工裁决)` +
        (backupBranch ? `,远端头已备份到 ${backupBranch}` : '') +
    ...
    return {
      ...
      conflicts, conflictCopies: [], backupBranch, backupPushed: backup?.pushed ?? false, ...(!this.ok(p) && { error: 'push' }),
    };
  }

  /**
   * 调度 dsh web 重启:用 schtasks 触发 ~/.dsh/restart-dsh-web.ps1(独立进程树)。
   * 不能直接调 powershell/杀进程:插件跑在 dsh web 服务器 Job Object 内,杀服务器会连带
   * 杀死自己(2026-09-10 实测);Task Scheduler 由 svchost 派生,脱离服务器 Job。
   * 返回值:true=已调度;false=未调度(缺脚本/调度失败/无远程)。
   * 幂等:每次同步只调度一次;restart 脚本本身杀旧实例→启动新实例。
   */
  scheduleRestartAfterRepair(cfg) {
    try {
      const restartPs1 = path.join(this.home, 'restart-dsh-web.ps1');
      if (!fs.existsSync(restartPs1)) {
        this.log('warn', `autoRestartAfterRepair 已开启,但找不到 ${restartPs1}(未部署重启脚本),本次不重启;可手动跑 fix-ungrouped-oneshot.mjs`);
        return false;
      }
      // 延迟调度:让当前同步的 HTTP 响应/日志先落地,再重启(否则响应没发出去进程就没了)
      const taskName = 'DSH-Sync-Repair-Restart';
      const taskTr = `powershell.exe -NoProfile -ExecutionPolicy Bypass -File "${restartPs1}"`;
      const create = spawnSync('schtasks', ['/create', '/tn', taskName, '/tr', taskTr, '/sc', 'once', '/st', '00:00', '/f'], {
        encoding: 'utf8', windowsHide: true, timeout: 15000,
      });
      if (create.status !== 0) {
        this.log('warn', `autoRestartAfterRepair 调度重启失败(schtasks create): ${(create.stderr || create.stdout || '').trim().split('\n')[0] || create.status}`);
        return false;
      }
      const runTask = spawnSync('schtasks', ['/run', '/tn', taskName], { encoding: 'utf8', windowsHide: true, timeout: 15000 });
      if (runTask.status !== 0) {
        this.log('warn', `autoRestartAfterRepair 调度重启失败(schtasks run): ${(runTask.stderr || runTask.stdout || '').trim().split('\n')[0] || runTask.status}`);
        return false;
      }
      this.log('info', '同步后自查已补登记会话,autoRestartAfterRepair 开启 → 已调度 dsh web 重启(新实例重建索引后「未分组」会话归位;浏览器会短暂断开,刷新恢复)');
      return true;
    } catch (e) {
      this.log('warn', 'autoRestartAfterRepair 调度重启异常: ' + String((e && e.message) || e));
      return false;
    }
  }
  ...
  async _syncOnce(reason, opts = {}) {
    ...
    // 同步前会话集 + 登记集:用于检测「本次同步拉取到新会话」或「有会话被新登记」→ 需重建 DSH 运行实例的内存索引
    const sessionsBefore = new Set(this.localSessionIds());
    const registeredBefore = this.registeredSessionIds();
    const rMain = await this._mainSync(reason, opts);
    if (rMain?.error) return rMain;
    // _mainSync may create and persist the remote on first use.
    const activeCfg = this.loadConfig();
    // 同步后自查:拉取/合并落盘的会话立即补进工作区登记表(不再落「未分组」),
    // 有变化就补一次提交+推送,让「会话↔工作区」对应关系随本次同步真正跨机收敛。
    // 即使远端旧版插件没登记、或登记被并集合并丢弃,这里也按会话 cwd 兜底归位。
    if (!(rMain && rMain.error)) {
      try {
        const fix = this.repairWorkspaceRegistry();
        const repairChanged = fix && (fix.repaired > 0 || fix.created > 0 || fix.deduped > 0 || fix.unarchived > 0);
        if (repairChanged) {
          const cm = await this.commitIfDirty(activeCfg, true);
          if (cm === 'committed' && activeCfg.remote) {
            const p = await this.runWithRetry('push', '主仓库', ['push', 'origin', activeCfg.branch], { timeoutMs: 180000, onProgress: (d) => this.liveTransfer('push', '主仓库', d) });
            if (!this.ok(p)) {
              this.log('error', `工作区登记补推失败: ${p.err.trim().split('\n')[0] || p.code}`);
              return { ...rMain, pushed: false, error: 'push', hint: '工作区登记已在本地保存，但没有上传成功；请检查网络后重试。' };
            }
            else this.log('info', `同步后自查:已把 ${fix.repaired} 个会话补进工作区` + (fix.created ? `、新建 ${fix.created} 个工作区` : '') + ',已提交并推送');
          } else if (cm === 'committed') {
            this.log('info', `同步后自查:已把 ${fix.repaired} 个会话补进工作区(未配置 remote,仅本地提交)`);
          }
        }
        // 重建索引触发:①本次同步拉取到新会话,或 ②有会话从「未登记」被补进登记(可能发生在同步前 repair),
        // 两种情况都会让运行中 dsh 的内存索引过期 → 需重启 dsh 让新实例重建 sessionPath 索引(否则侧栏持续显示「未分组」)
        const newSessions = [...this.localSessionIds()].filter((id) => !sessionsBefore.has(id));
        const newlyRegistered = [...this.registeredSessionIds()].filter((id) => !registeredBefore.has(id));
        const reindexNeeded = newSessions.length > 0 || newlyRegistered.length > 0;
        if (reindexNeeded && activeCfg.autoRestartAfterRepair === true) {
          this.log('info', `同步后自查:检测到需重建索引(新会话 ${newSessions.length} 个 / 新登记 ${newlyRegistered.length} 个),重启 dsh 以消除「未分组」`);
          this.scheduleRestartAfterRepair(activeCfg);
        }
      } catch (e) {
        this.log('warn', '同步后工作区自查异常(不影响同步结果): ' + String(e?.message || e));
      }
    }
    let workspaces = { skipped: 'not-run', total: 0, synced: 0, pushed: 0, pulled: 0, errors: [] };
    try {
      workspaces = await this.syncAllWorkspaces(activeCfg, reason, opts);
    } catch (e) {
      workspaces = { skipped: 'error', total: 0, synced: 0, pushed: 0, pulled: 0, errors: [{ id: '', title: '', path: '', error: String(e?.message || e) }] };
    }
    try { this.reportOrphanWorkspaceRepos(activeCfg); } catch { /* 仅提示,失败忽略 */ }
    ...
    const repoSize = this.repoSizeBytes();    return {
      ...rMain, workspaces,
  ...
  syncOnce(reason = 'manual', opts = {}) {
    ...
    this.inFlight = this._syncOnce(reason, opts)
      ...
      .finally(() => {
        this.inFlight = null;
        this._conflictsCache = null; // 本轮同步可能新增/清理了冲突拷贝 → 让下次扫描重新读盘
      });
  ...
  flushSync() {
    const cfg = this.loadConfig();
    // Exit hooks cannot await gh; never upload a remote that this process has not verified.
    if (cfg.remote !== this._verifiedPrivateRemote) cfg.remote = '';
    ...
    if (cfg.remote && cfg.remote === this._verifiedPrivateRemote) {
    ...
    if (cfg.workspaceSync !== false && !this.gitMissing) {
      for (const ws of this.workspaces()) {
        try {
          ...
          // 只写 worktree 配置(真实文件夹可能已按本机覆盖换过位置);不改 core.bare(init 时已设)
          const curWt = this.gitWSync(gitdir, realPath, ['config', 'core.worktree']);
          if (!this.ok(curWt) || curWt.out.trim() !== realPath) {
