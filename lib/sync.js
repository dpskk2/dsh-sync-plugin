...
export class SyncEngine {
  ...
  async _syncOnce(reason, opts = {}) {
    ...
    // 同步前会话集:用于检测「本次同步拉取到新会话」→ 需重建 DSH 运行实例的内存索引(见同步后自查)
    ...
    if (!(rMain && rMain.error)) {
      try {
        ...
        // 重建索引触发:①补登记有变化,或 ②本次同步拉取到了新会话(登记已在盘上、但运行中 dsh 的内存索引未刷新)
        // → 都需重启 dsh 让新实例重建 sessionPath 索引(否则侧栏持续显示「未分组」——2026-09-11 根因)
        const newSessions = [...this.localSessionIds()].filter((id) => !sessionsBefore.has(id));
        if ((repairChanged || newSessions.length > 0) && cfg.autoRestartAfterRepair === true) {
          if (!repairChanged && newSessions.length > 0) {
            this.log('info', `同步后自查:本次拉取到 ${newSessions.length} 个新会话(登记已在盘上),重启 dsh 重建索引以消除「未分组」`);
          }
