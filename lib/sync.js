...
/**
 * 内置 .gitignore:只排除「每台机器各自的状态 / 可再生依赖」,其余全部同步。
 *
 * 设计取向:本插件是「个人私有仓库」同步工具(remote 默认指向你自己的私有 repo),
 * 因此 **settings.yaml(字号/模型/默认模型/第三方 provider 配置)与
 * .credentials.yaml(API 密钥)默认一起同步** —— 换电脑后设置与密钥直接带过去,
 * 不再出现字号复位、provider/模型选择丢失、密钥要重新登录的问题。
...
/**
 * 旧版插件(≤0.9.x)生成的「配置/密钥忽略」条目:v0.10 起个人私有仓库默认同步
 * 设置与密钥,这些条目已从 BUILTIN_IGNORE 移除;升级后旧 .gitignore 里的残留
 * 由 ensureRepo 的迁移逻辑清理(见下)。
 */
export const LEGACY_IGNORE_REMOVALS = ['.credentials.yaml', 'settings.yaml'];
/** 与上述条目相伴的旧版注释行(按新措辞重写,不整行删除,避免破坏用户阅读) */
export const LEGACY_IGNORE_COMMENT_REWRITES = {
...
export class SyncEngine {
  ...
  git(args, timeoutMs = 120000, onProgress = null) {
    return new Promise((resolve) => {
      ...
      let child;
      try {
        child = spawn('git', args, { cwd: this.home, windowsHide: true, env: this._gitEnv });
      } catch (e) {
  ...
  async ensureRepo(cfg) {
    ...
    if (!fs.existsSync(ignorePath)) {
      ...
      // v0.10 迁移:旧版(≤0.9.x)把 settings.yaml / .credentials.yaml 写进了 .gitignore
      // (密钥永不上传的旧取向)。个人私有仓库同步改为默认同步设置与密钥后,清理这两条
      // 插件生成的残留,让升级用户下次同步就把配置与密钥带上(用户手工加的其他条目保留)。
      try {
        ...
        if (next !== cur) {
          ...
          if (changed.length) this.log('info', `已迁移 .gitignore:v0.10 起设置与密钥默认同步,移除旧条目 ${changed.join(' / ')}`);
        }
      } catch { /* ignore */ }
    }
  ...
  async fetchWithRetry(args, timeoutMs = 180000, onProgress = null) {
    const withProg = [...args, '--progress'];
    const r1 = await this.git(withProg, timeoutMs, onProgress);
    this.recordTransfer('fetch', r1, '主仓库');
    if (this.ok(r1)) return r1;
    if (this.isAuthError(r1.err)) {
      this.log('warn', `git fetch 需要凭据但当前不可用(已禁用交互弹窗,故不再弹 GCM)。请先 gh auth login 或配置 PAT(见 README「凭据」),再点同步。${String(r1.err).trim().split('\n')[0]}`);
      return r1;
    }
    await new Promise((res) => setTimeout(res, 1200));
    const r2 = await this.git(withProg, timeoutMs, onProgress);
    this.recordTransfer('fetch', r2, '主仓库');
    if (this.ok(r2)) this.log('info', 'git fetch 首次失败(网络抖动)后重试成功');
    return r2;
  }
  /** 工作区版 fetch 重试 */
  async fetchWithRetryW(gitdir, realPath, args, timeoutMs = 180000, onProgress = null) {
    const withProg = [...args, '--progress'];
    const label = '工作区 ' + path.basename(realPath || '');
    const r1 = await this.gitW(gitdir, realPath, withProg, timeoutMs, onProgress);
    this.recordTransfer('fetch', r1, label);
    if (this.ok(r1)) return r1;
    if (this.isAuthError(r1.err)) {
      this.log('warn', `工作区 ${label} fetch 需要凭据但当前不可用(已禁用交互弹窗): ${String(r1.err).trim().split('\n')[0]}`);
      return r1;
    }
    await new Promise((res) => setTimeout(res, 1200));
    const r2 = await this.gitW(gitdir, realPath, withProg, timeoutMs, onProgress);
    this.recordTransfer('fetch', r2, label);
    if (this.ok(r2)) this.log('info', `工作区 ${label} git fetch 首次失败(网络抖动)后重试成功`);
    return r2;
  }

  /** push 带一次快速重试:网络瞬时抖动时重试一次常能成功(已成功的 push 重试=Everything up-to-date,安全) */
  async pushWithRetry(args, timeoutMs = 180000, onProgress = null) {
    const withProg = [...args, '--progress'];
    const r1 = await this.git(withProg, timeoutMs, onProgress);
    this.recordTransfer('push', r1, '主仓库');
    if (this.ok(r1)) return r1;
    if (this.isAuthError(r1.err)) {
      this.log('warn', `git push 需要凭据但当前不可用(已禁用交互弹窗,故不再弹 GCM)。请先 gh auth login 或配置 PAT(见 README「凭据」)。${String(r1.err).trim().split('\n')[0]}`);
      return r1;
    }
    await new Promise((res) => setTimeout(res, 1200));
    const r2 = await this.git(withProg, timeoutMs, onProgress);
    this.recordTransfer('push', r2, '主仓库');
    if (this.ok(r2)) this.log('info', 'git push 首次失败(网络抖动)后重试成功');
    return r2;
  }
  /** 工作区版 push 重试 */
  async pushWithRetryW(gitdir, realPath, args, timeoutMs = 180000, onProgress = null) {
    const withProg = [...args, '--progress'];
    const label = '工作区 ' + path.basename(realPath || '');
    const r1 = await this.gitW(gitdir, realPath, withProg, timeoutMs, onProgress);
    this.recordTransfer('push', r1, label);
    if (this.ok(r1)) return r1;
    if (this.isAuthError(r1.err)) {
      this.log('warn', `工作区 ${label} push 需要凭据但当前不可用(已禁用交互弹窗): ${String(r1.err).trim().split('\n')[0]}`);
      return r1;
    }
    await new Promise((res) => setTimeout(res, 1200));
    const r2 = await this.gitW(gitdir, realPath, withProg, timeoutMs, onProgress);
    this.recordTransfer('push', r2, label);
    if (this.ok(r2)) this.log('info', `工作区 ${label} git push 首次失败(网络抖动)后重试成功`);
    return r2;
  }
  ...
  async backupRemoteHeadWs(gitdir, realPath, remoteHead) {
    ...
    if (!this.ok(cb)) return null;
    await this.pushWithRetryW(gitdir, realPath, ['push', 'origin', branchName], 180000, (d) => this.liveTransfer('push', '工作区 ' + path.basename(realPath || ''), d));
    return branchName;
  }
  ...
  async syncWorkspace(ws, cfg, reason, opts) {
    ...
    const gitdir = this.workspaceGitDir(id);
    const branch = this.workspaceBranchOf(cfg, id);
    const createdHere = !fs.existsSync(realPath);
    ...
    let remoteHead = null;
    if (cfg.remote) {
      const rf = await this.fetchWithRetryW(gitdir, realPath, ['fetch', 'origin', '--prune'], 180000, (d) => this.liveTransfer('fetch', '工作区 ' + (label || id), d));
    ...
    if (remoteHead === null) {
      ...
      const p = await this.pushWithRetryW(gitdir, realPath, ['push', '-u', 'origin', branch], 180000, (d) => this.liveTransfer('push', '工作区 ' + (label || id), d));
    ...
    if (localHead === remoteHead) {
      if (committed) {
        const p = await this.pushWithRetryW(gitdir, realPath, ['push', 'origin', branch], 180000, (d) => this.liveTransfer('push', '工作区 ' + (label || id), d));
  ...
  decodeLogBytes(buf) {
    ...
    if (buf.subarray(0, 4).equals(magic) && typeof zstdDecompressSync === 'function') {
      ...
      while (i <= buf.length - 4) {
        ...
        if (idx === -1) break;
        const i2 = buf.indexOf(magic, idx + 4);
        const frame = i2 === -1 ? buf.subarray(idx) : buf.subarray(idx, i2);
        try { out = Buffer.concat([out, zstdDecompressSync(frame)]); } catch { break; }
  ...
  async _mainSync(reason, opts = {}) {
    ...
    if (hasRemote) {
      ...
      const rf = await this.fetchWithRetry(['fetch', 'origin', '--prune'], 180000, (d) => this.liveTransfer('fetch', '主仓库', d));
      if (!this.ok(rf)) {
    ...
    if (remoteHead === null) {
      ...
      const p = await this.pushWithRetry(['push', '-u', 'origin', cfg.branch], 180000, (d) => this.liveTransfer('push', '主仓库', d));
      if (this.ok(p)) {
    ...
    if (localHead === remoteHead) {
      if (committed) {
        ...
        const p = await this.pushWithRetry(['push', 'origin', cfg.branch], 180000, (d) => this.liveTransfer('push', '主仓库', d));
