  ...
  async _syncOnce(reason, opts = {}) {
    ...
    if (!(rMain && rMain.error)) {
      try {
        ...
        const changed = fix && (fix.repaired > 0 || fix.created > 0 || fix.deduped > 0);
        if (changed) {
          ...
          if (cm === 'committed' && cfg.remote) {
            ...
            else this.log('info', `同步后自查:已把 ${fix.repaired} 个会话补进工作区` + (fix.created ? `、新建 ${fix.created} 个工作区` : '') + ',已提交并推送(若侧栏仍在「未分组」,重启 dsh 重新加载登记表即可)');
          } else if (cm === 'committed') {
            this.log('info', `同步后自查:已把 ${fix.repaired} 个会话补进工作区(未配置 remote,仅本地提交)`);
          }
          // autoRestartAfterRepair:补登记有变化时自动重启 dsh,让新实例重建 sessionPath 索引
          // (否则修复只落在磁盘,运行中实例的 mutate() 会在下次工作区写操作时再次剪掉 —— 2026-09-11 根因)。
          if (cfg.autoRestartAfterRepair === true) {
