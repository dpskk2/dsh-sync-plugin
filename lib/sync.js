...
export class SyncEngine {
  ...
  async syncWorkspace(ws, cfg, reason, opts) {
    ...
    return {
      ...
      conflicts, conflictCopies: [], backupBranch, path: realPath,
    };
  }
  ...
  async _mainSync(reason, opts = {}) {
    ...
    return {
      ...
      conflicts, conflictCopies: [], backupBranch,
    };
  }
  ...
  async _syncOnce(reason, opts = {}) {
    ...
    if (!(rMain && rMain.error)) {
      try {
        ...
        if (repairChanged) {
          const cm = await this.commitIfDirty(cfg, true);
          if (cm === 'committed' && cfg.remote) {
            const p = await this.runWithRetry('push', '主仓库', ['push', 'origin', cfg.branch], { timeoutMs: 180000, onProgress: (d) => this.liveTransfer('push', '主仓库', d) });
            if (!this.ok(p)) this.log('error', `工作区登记补推失败: ${p.err.trim().split('\n')[0] || p.code}`);
    ...
    try { this.reportOrphanWorkspaceRepos(cfg); } catch { /* 仅提示,失败忽略 */ }
